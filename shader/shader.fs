...
uniform highp vec3 lightPositions;
uniform highp vec3 lightColors;
...
void main()
{
    ...
    vec3 diffuse = pow(texture(DIFFUSE1, TexCoords).rgb, vec3(2.2));
    float metallic  = texture(METALLIC1, TexCoords).r;
    float roughness = texture(ROUGHNESS1, TexCoords).r;
    float ao = texture(AO1, TexCoords).r;

    vec3 tangentNormal = texture(NORMAL1, TexCoords).xyz * 2.0 - 1.0;
